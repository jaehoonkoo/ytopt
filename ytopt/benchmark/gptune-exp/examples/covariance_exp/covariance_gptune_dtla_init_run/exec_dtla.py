...
task_s = {}
task_s['s']  = [[80,100],' -DSMALL_DATASET'] 
task_s['sm'] = [[160,180],' -DSM_DATASET'] 
task_s['m']  = [[240,260],' -DMEDIUM_DATASET'] 
task_s['ml'] = [[720,830],' -DML_DATASET'] 
task_s['l']  = [[1200,1400],' -DLARGE_DATASET'] 
task_s['xl'] = [[2600,3000],' -DEXTRALARGE_DATASET'] 

input_sizes= {}
input_sizes[(80,100)]  = [' -DSMALL_DATASET'] 
input_sizes[(160,180)] = [' -DSM_DATASET'] 
input_sizes[(240,260)] = [' -DMEDIUM_DATASET'] 
input_sizes[(720,830)] = [' -DML_DATASET'] 
input_sizes[(1200,1400)] = [' -DLARGE_DATASET'] 
input_sizes[(2600,3000)] = [' -DEXTRALARGE_DATASET']
################################### 
...
def objectives(point: dict):
    ...
    ### old task
        ...
        print ("Task: ", point[tuple(task)])
        print ("point",point)
        ret = model_functions[tuple(task)](point)
        print ("model ret: ", ret)     
        return ret
...
def create_gptune(Kernel_name, nodes, cores, n_param, param_start, param_name, i_min, i_max, task_name):
    ...
    Tuning_params = [p0,p1,p2,p3,p4,p5,p6,p7,p8]
#     r = Real(float("-Inf"), float("Inf"), name="y")
    y = Real(float(0), float("Inf"), name="y")
...
if __name__ == "__main__":
    ...
    if ntask == 1:
        ...
        giventask = [i_target, task_s['s'], task_s['m'], task_s['l']] 
    else:
    ...
    for i in range(1,len(giventask),1):
        ...
        meta_dict = {
            ...
            "input_space": i_space,
            "parameter_space": [
        {"name": "p0","transformer": "onehot","type": "categorical","categories": ["4","5","6","7","8"]},
        {"name": "p1","transformer": "onehot","type": "categorical","categories": ["100","200","400","640","800","1000","1280","1600","2000"]},
        {"name": "p2","transformer": "onehot","type": "categorical","categories": ["#pragma clang loop unrolling full"," "]},
        {"name": "p3","transformer": "onehot","type": "categorical","categories": ["#pragma omp parallel for"," "]},
        {"name": "p4","transformer": "onehot","type": "categorical","categories": ["2","4","8","16","32","64","96","128","256"]},
        {"name": "p5","transformer": "onehot","type": "categorical","categories": ["2","4","8","16","32","64","96","128","256"]},
        {"name": "p6","transformer": "onehot","type": "categorical","categories": ["10","20","40","64","80","100","128","160","200"]},
        {"name": "p7","transformer": "onehot","type": "categorical","categories": ["compact","scatter","balanced","none","disabled","explicit"]},
        {"name": "p8","transformer": "onehot","type": "categorical","categories": ["cores","threads","sockets"]},],
            "output_space": [{"name":"y","type":"real","transformer":"identity"}],
            "loadable_machine_configurations":{},
        }
        f = open(f"exp/gptune/{model_file_tl[:-4]}-{Run_index}/{Kernel_name}.json")
