...
def myobj(point: dict):
  ...
  np.save(dir_path+'/exe_times_sdv_itr_time.npy',exe_times)
...
# name of csv file 
filename = "results_sdv_itr_time.csv"
...
#     for idx in range(N_infer):
    ...
    while eval_master < Max_evals:         
        ...
        conditions = {'input': input_sizes['sm'][0]}
        # conditions = pd.DataFrame({'gender': ['M', 'M', 'M', 'F', 'F', 'F']})
        ss1 = model.sample(1000,conditions=conditions)
...
#         print (new_kde['speedup'])
        ...
        stop = False
        while stop == False:
#             print (eval_master, Max_evals)
#             print (eval_update, max_evals)
...
#                 print (sample_point)
                ...
                csvfile.flush()
                row_prev = row
#                 if False:# float(row[1][-1]) <= float(o3p_time['sm'][0] / res): 
